...
Typically, the number of detections induced by the $\mathrm{KCl}$ sample is counted during a certain time period, using a counter/timer. The count rate *R* is obtained by dividing the count total $N_{count}$ by the measurement time $\Delta t: R = N_{count}/\Delta t$. Since the count rate is not equal to the activity, it is therefore expressed in $s^{-1}$ and not in Bq. The count rate is recalculated into the activity by making several corrections. The discussion below is limited to those corrections that give a significant contribution.
